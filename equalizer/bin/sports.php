...
$result3=mysqli_query($con,'SELECT* FROM articles_tbl where category_id="12" LIMIT '.$start.','.$limit.'');
...
$art_date=$row3['article_date'];
	...
	echo '</span></div></div><div class="col-lg-9 col-md-9 col-sm-9  blog-content">
		 <h1>
		 <a href="sports_item.php?id='.$row3['article_id'].'">'.$row3['article_title'].'</a>
		 </h1><div class="media"> <a href="sports_item.php?id='.$row3['article_id'].'" class="media-left"> <img class="img-blog" alt="" src="images/announcement/'.$row3['article_photo'].'"> </a>
							  <div class="media-body">
							  <p id="side_post">'.substr(nl2br(htmlentities($row3['article_headline'])),0,230).'...</p></div>					
							</div>';
	echo '<a class="btn btn-primary readmore" href="sports_item.php?id='.$row3['article_id'].'">Read More 
...
$rows=mysqli_num_rows(mysqli_query($con,'select * from articles_tbl where category_id="12"'));//calculate total page number for the given table in the database 
