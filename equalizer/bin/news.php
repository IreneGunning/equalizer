...
$result3=mysqli_query($con,'SELECT* FROM articles_tbl where category_id="1" LIMIT '.$start.','.$limit.'');
...
$art_date=$row3['article_date'];
	...
	echo '</span></div></div><div class="col-lg-9 col-md-9 col-sm-9  blog-content">
		 <h1>
		 <a href="news_item.php?id='.$row3['article_id'].'">'.$row3['article_title'].'</a>
		 </h1><div class="media"> <a href="news_item.php?id='.$row3['article_id'].'" class="media-left"> <img class="img-blog" alt="" src="images/announcement/'.$row3['article_photo'].'"> </a>
							  <div class="media-body">
							  <p id="side_post">'.substr(nl2br(htmlentities($row3['article_headline'])),0,230).'...</p></div>					
							</div>';
	echo '<a class="btn btn-primary readmore" href="news_item.php?id='.$row3['article_id'].'">Read More 
...
$rows=mysqli_num_rows(mysqli_query($con,'select * from articles_tbl where category_id="1"'));//calculate total page number for the given table in the database 
      ...
      <div class="fixed col-lg-4 col-md-4 col-sm-4">
        ...
        <div class="single_sidebar">
          ...
          <div class="latest_post_container">
            <ul class="spost_nav">
			<?php
				$result=mysqli_query($con,'SELECT * FROM articles_tbl where category_id="1" ORDER BY `articles_tbl`.`article_date` limit 4');
